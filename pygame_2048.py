...
class Board:
    ...
    def up(self):
        ...
        #Check for two pairs in a column. Take care of both, then mark the column as done.
        for j in range(4):
            ...
            for i in range(1,4):
                for j in range(4):
                    if self.board[i][j] != '.':
                        ...
                        # If the space above a number is empty, swap the cells
                        if self.board[i-1][j] == '.':
                            self.board[i][j], self.board[i-1][j] = self.board[i-1][j],self.board[i][j]
                            move_count += 1

                        # If the cell and the one above are the same value and a combination in
                        # this column hasn't already occurred, multiply the one above by 2
                        # and get rid of the current one's value.
